...
def main(WIT_ID, num_averaged_years, start_date, end_date, threshold_factor, top_left, bottom_left, bottom_right, top_right):
    ...
    print('https://climateserv.servirglobal.net/chirps/submitDataRequest/?datatype=0&begintime=%s&endtime=%s&intervaltype=%s&operationtype=%s&dateType_Category=default&isZip_CurrentDataType=%s&geometry={"type":"Polygon","coordinates":[%s]}' % (start_date.strftime("%m/%d/%Y"), end_date.strftime("%m/%d/%Y"), INTERVAL_TYPE, OPERATION_TYPE, ZIP, square))

    request_url = 'https://climateserv.servirglobal.net/chirps/submitDataRequest/?datatype=0&begintime=%s&endtime=%s&intervaltype=%s&operationtype=%s&dateType_Category=default&isZip_CurrentDataType=%s&geometry={"type":"Polygon","coordinates":[%s]}' % (start_date.strftime("%m/%d/%Y"), end_date.strftime("%m/%d/%Y"), INTERVAL_TYPE, OPERATION_TYPE, ZIP, square)
    ...
    print('https://climateserv.servirglobal.net/chirps/getDataRequestProgress/?id=%s' % job_id)    
